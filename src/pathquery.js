(function(undefined) {
	...
	/*
		Local pathquery object to set up and return
	*/

		pathquery = function(source, parseType) {
			var nodes, tmp;
		...
		p = pathquery.prototype,

		// Returned public-facing function
		$ = function(selOrElements) {
			return new pathquery(selOrElements);
		},
		...
		*/
		...
		/*
			Walk through pathquery/NodeList/HTMLCollection/Array object's HTMLElements, optionally filtering it to create a new pathquery object.
		...
		*/
		zQ_fn_iterate = function(zQ, fn, ret) {
			var elems = zQ instanceof pathquery ? zQ.get() : zQ,
		...
		zQ_fn_matches = function(elem, match) {
			/*
				Returns if
					(String) selector,
					(Object) Element, or
					at least one Element inside (Object) HTMLCollection/NodeList/pathquery
			...
			*/
			...
			// Assuming {match} is pathquery or HTMLCollection or NodeList
			var result = false;
		...
		zQ_fn_appendPrepend = function(mode, switch_roles, elems, nodes) {
			/*
				...
				When using as appendTo or prependTo, it returns a new pathquery object containing all the HTMLElements that were appended (including clones), not just the original ones.

				When using as append or prepend, it returns the original pathquery object as usual.
			*/
	...
	*/
	...
	p.add = function(elems) {
		if (elems) {
			...
			// Create new pathquery element from current pathquery Element objects and provided Element
			...
			/*
				Create new pathquery from current pathquery HTMLElement objects and...
				1) (if string) elements matching string selector in whole document
				2) (if pathquery) HTMLElement objects from adding pathquery object
				3) (other) {elems}
			*/
			var arr = elems instanceof pathquery ? elems.get() : typeof elems == TYPEOF_STRING ? qsa(elems) : elems;
	...
	*/
	p.find = function(sel) {
		...
		while (elem = toCheck.shift()) {
			...
			if (isMatching) {
				...
				// Add the matching element if a new pathquery object is wanted
				else {
					toRet.push(elem);
				}
			}
	...
	p.filter = function(selOrFn) {
		/*
			Provide a function to manually decide what to include in the new pathquery object by returning HTMLElements or arrays containing them inside the function

			Provide a selector or any valid HTMLElement-related object (pathquery, HTMLCollection, NodeList, HTMLElement) to filter based on them
		*/
	...
	*/
	...
	/*
		...
		The first element in the pathquery elements must be a <template>. Only the first element's contents is retrieved.
	*/
	...
	/*
		Argument options:
			-- an existing array: appends each element's inner HTML to the end of the array, returns the pathquery object
	...
	*/
	...
	/*
		...
		or ALL event listeners CACHED by pathquery with the type provided.
	*/
	p.off = function(eventTypes, eventFunction) {
		eventTypes = eventTypes.split(zQ_set_regexp_whitespace);
		zQ_fn_iterate(this, function(elem) {
			// Get any pathquery properties on this element
	...
	p.one = function(eventTypes, eventFunction) {
		eventTypes = eventTypes.split(zQ_set_regexp_whitespace);
		zQ_fn_iterate(this, function(elem) {
			// Prepare HTMLElement object's pathquery eventListener property
			var listeners = elem[zQ_set_prop_eventListeners] = elem[zQ_set_prop_eventListeners] || [];
