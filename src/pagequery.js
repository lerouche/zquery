(function (undefined) {
  ...
  var
    ...
    /*
      Local pagequery object to set up and return
    */

    pagequery = function (source, parseType) {
      var nodes, tmp;
    ...
    p = pagequery.prototype,

    // Returned public-facing function
    $ = function (selOrElements) {
      return new pagequery(selOrElements);
    },
    ...
    */
    ...
    /*
      Walk through pagequery/NodeList/HTMLCollection/Array object's HTMLElements, optionally filtering it to create a new pagequery object.
    ...
    */
    zQ_fn_iterate = function (zQ, fn, ret) {
      var elems = zQ instanceof pagequery ? zQ.get() : zQ,
    ...
    zQ_fn_matches = function (elem, match) {
      /*
        Returns if
          (String) selector,
          (Object) Element, or
          at least one Element inside (Object) HTMLCollection/NodeList/pagequery
      ...
      */
      ...
      // Assuming {match} is pagequery or HTMLCollection or NodeList
      var result = false;
    ...
    zQ_fn_appendPrepend = function (mode, switch_roles, elems, nodes) {
      /*
        ...
        When using as appendTo or prependTo, it returns a new pagequery object containing all the HTMLElements that were appended (including clones), not just the original ones.

        When using as append or prepend, it returns the original pagequery object as usual.
      */
      ...
      zQ_fn_iterate(elems, function (elem, i, t) {
        zQ_fn_iterate(nodes, function (node) {
          ...
          if (mode == 1) {
            ...
            (mode ? elem.parentNode : elem).insertBefore(node, (!mode ?
              elem.childNodes[0] :
              mode == 2 ? elem : elem.nextSibling) || nil); // Insert before all nodes, even text (so don't use .children); null must be passed if no childNodes to avoid errors
          }
        }, false);
      }, false);
  ...
  */
  ...
  p.add = function (elems) {
    if (elems) {
      ...
      // Create new pagequery element from current pagequery Element objects and provided Element
      ...
      /*
        Create new pagequery from current pagequery HTMLElement objects and...
        1) (if string) elements matching string selector in whole document
        2) (if pagequery) HTMLElement objects from adding pagequery object
        3) (other) {elems}
      */
      var arr = elems instanceof pagequery ? elems.get() : typeof elems == TYPEOF_STRING ? qsa(elems) : elems;
  ...
  */
  p.find = function (sel) {
    ...
    while (elem = toCheck.shift()) {
      ...
      if (isMatching) {
        ...
        // Add the matching element if a new pagequery object is wanted
        else {
          toRet.push(elem);
        }
      }
  ...
  p.filter = function (selOrFn) {
    /*
      Provide a function to manually decide what to include in the new pagequery object by returning HTMLElements or arrays containing them inside the function

      Provide a selector or any valid HTMLElement-related object (pagequery, HTMLCollection, NodeList, HTMLElement) to filter based on them
    */
  ...
  */
  p.children = function (sel) {
    ...
    if (typeof sel == TYPEOF_STRING &&
        ((requireAll = (requireDeep = (firstChar = sel[0]) == "!") || firstChar == "*") || firstChar == "?")) {
      sel = sel.slice(1);
  ...
  /*
    ...
    The first element in the pagequery elements must be a <template>. Only the first element's contents is retrieved.
  */
  ...
  p.databind = function (data, notLive) {
    if (notLive) {
      return zQ_fn_iterate(this, function (elem) {
        if (elem instanceof EN) {
          ...
          html = html.replace(zQ_set_regexp_handlebars, function (_, bar, propName) {
            ...
            if (bar == "{{") {
              propValue = zQ_fn_escape_HTML(propValue);
            }
            return propValue;
          });
      ...
      var TEMPLATE_TEXT_PROP_NAME = "pqdbt",
        templateText,
        toIterate = this.get(),
        elemToCheck,
        assoc = {},
        ...
        set: function (target, property, value) {
          ...
          if (associations) {
            zQ_fn_iterate(associations, function (attrOrTextnode) {
              attrOrTextnode[attrOrTextnode instanceof Attr ?
                "value" :
                "textContent"] = attrOrTextnode[TEMPLATE_TEXT_PROP_NAME]
  ...
  /*
    Argument options:
      -- an existing array: appends each element's inner HTML to the end of the array, returns the pagequery object
  ...
  */
  ...
  /*
    ...
    or ALL event listeners CACHED by pagequery with the type provided.
  */
  p.off = function (eventTypes, eventFunction) {
    eventTypes = eventTypes.split(zQ_set_regexp_whitespace);
    zQ_fn_iterate(this, function (elem) {
      // Get any pagequery properties on this element
  ...
  p.one = function (eventTypes, eventFunction) {
    eventTypes = eventTypes.split(zQ_set_regexp_whitespace);
    zQ_fn_iterate(this, function (elem) {
      // Prepare HTMLElement object's pagequery eventListener property
      var listeners = elem[zQ_set_prop_eventListeners] = elem[zQ_set_prop_eventListeners] || [];
  ...
  p.hasFocus = function () {
    return this.get()[ai_some](function (elem) {
      ...
      return elem == d.activeElement && (!d.hasFocus || d.hasFocus()) &&
             !!(elem.type || elem.href || elem.tabIndex >= 0);
    });
  };
